...
with open("plain_trace_2.txt", "w") as ft:
	print("create trace")
...
with open("plain_qeps.pkl","rb") as file:
...
for file in file_list:
	with open("plain_trace_2.txt", "a+") as ft:
		ft.write("start\n")
	...
	for n_rel, rels in enumerate(rel_matches):
		...
		if rels[0] != root_rel:
			...
			if len(rels) == 2 and not "#" in insert_key.split("/")[-1]:
				...
				#reference_branch = reference_branch.getNext(insert_key.split("/")[-1], selectivity, skip_bound)
				replace_operator = reference_branch.getNext(insert_key.split("/")[-1], selectivity).getRecommended()
				with open("plain_trace_2.txt", "a+") as ft:
					ft.write("dummy_branch = reference_branch getNext {} \n".format(insert_key.split("/")[-1]))
				with open("plain_trace_2.txt", "a+") as ft:
			...
			with open("plain_trace_2.txt", "a+") as ft:
				ft.write("reference_branch = reference_branch getNext {} \n".format(insert_key.split("/")[-1]))			
			#new_branch, update_branch = reference_branch[insert_key].update(insert_key, feedback_list[n_rel], selectivity, f_l[n_rel], file, rel_info)
			feedback_alt = feedback_alt.replace(f_l[n_rel]+"("+(" ").join(rels)+")",replace_operator+"("+(" ").join(rels)+")")
...
print("finished plain_trace_2.txt")
