...
with open("filter_trace_1.txt", "w") as ft:
	print("create trace")
...
for file in file_list:
	...
	with open("FilterExp/"+file.split("/")[-1].replace("sql","pkl"),"rb") as filterFile:
		filterdict =  pickle.load(filterFile)


	for key in filterdict.keys():
		alias["$"+key+"$"]="$"+alias[key]+"$"+"*"+filterdict[key]+"*"
	the_keys = [k for k in alias.keys()]
	for key in the_keys:
		if "$" in key or"$"+key+"$" in the_keys:
			continue
		if not "$" in key:
			alias["$"+key+"$"]="$"+alias[key]+"$"
	...
	with open("filter_trace_1.txt", "a+") as ft:
		ft.write("reference_branch = plan_cache\n")
	...
	for n_match, m in enumerate(match):
		...
		if row_dict[" ".join(rels).replace("$","")] == 0:
	...
	for n_rel, rels in enumerate(rel_matches):
		...
		if rels[0] != root_rel:
			...
			selectivity = sel_dict[insert_rels[-1].replace("$","")]

			if len(rels) == 2:
				sel_1 = sel_dict[insert_rels[-2].replace("$","")]
				sel_2 = sel_dict[insert_rels[-1].replace("$","")]
				replace_operator = reference_branch.getNext("#"+insert_key.split("/")[-2], sel_1).getNext("#"+insert_key.split("/")[-1], sel_2).getRecommended()
				with open("filter_trace_1.txt", "a+") as ft:
					ft.write("dummy_branch = reference_branch getNext {} \n".format("#"+insert_key.replace(" ","-").split("/")[-2]))
				with open("filter_trace_1.txt", "a+") as ft:
					ft.write("dummy_branch = reference_branch getNext {} \n".format("#"+insert_key.replace(" ","-").split("/")[-1]))
				with open("filter_trace_1.txt", "a+") as ft:
					ft.write("dummy_branch getRecommended\n")	
				reference_branch.updateKey("#"+insert_key.split("/")[-2], [0,0], sel_1, immutable = skip_bound)
				with open("filter_trace_1.txt", "a+") as ft:
					ft.write("reference_branch updateKey {} {} {} \n".format("#"+insert_key.replace(" ","-").split("/")[-2], 0,0))
				reference_branch=reference_branch.getNext("#"+insert_key.split("/")[-2], sel_1)
				with open("filter_trace_1.txt", "a+") as ft:
					ft.write("reference_branch = reference_branch getNext {} \n".format("#"+insert_key.replace(" ","-").split("/")[-2]))
				branch_before = reference_branch

			else:
				replace_operator = reference_branch.getNext("#"+insert_key.split("/")[-1], selectivity).getRecommended()
				with open("filter_trace_1.txt", "a+") as ft:
					ft.write("dummy_branch = reference_branch getNext {}  \n".format("#"+insert_key.replace(" ","-").split("/")[-1], selectivity))
				with open("filter_trace_1.txt", "a+") as ft:
					ft.write("dummy_branch getRecommended\n")
			if feedback_list[n_rel][0] + feedback_list[n_rel][1] > 0: 
				...
				with open("filter_trace_1.txt", "a+") as ft:
					ft.write("reference_branch updateKey {} {} {} \n".format("#"+insert_key.replace(" ","-").split("/")[-1], feedback_list[n_rel][0],feedback_list[n_rel][1]))			
			reference_branch=reference_branch.getNext("#"+insert_key.split("/")[-1], selectivity)
			with open("filter_trace_1.txt", "a+") as ft:
				ft.write("reference_branch = reference_branch getNext {} \n".format("#"+insert_key.replace(" ","-").split("/")[-1]))
			feedback_alt = feedback_alt.replace(f_l[n_rel]+"("+(" ").join(rels).replace("$","")+")",replace_operator+"("+(" ").join(rels).replace("$","")+")")
...
with open("filterAware_qeps.pkl","wb") as file:
	pickle.dump(plan_cache,file)

print("finished filter_trace_1.txt")
