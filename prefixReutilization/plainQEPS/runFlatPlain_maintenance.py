...
path_dict = {}
def getPrefixCount(node, prefix_len, prefix):
	prefix += node.key + "/"
	key_cnt = 1
	if prefix_len not in path_dict.keys():
		path_dict[prefix_len] = node.counter
	elif node.counter > path_dict[prefix_len]:
		path_dict[prefix_len] = node.counter
	for key in node.child_nodes.keys():
		for interval in node.child_nodes[key].keys():
			getPrefixCount(node.child_nodes[key][interval], prefix_len+1, prefix)
	...
	for n_rel, rels in enumerate(rel_matches):
		...
		if rels[0] != root_rel:
			...
			if len(rels) == 2:
				...
				replace_operator = second_reference.getNext(insert_key.split("/")[-2], sel_1).getNext(insert_key.split("/")[-1], sel_2).getRecommended()
				second_reference.updateKey(insert_key.split("/")[-2], [0,0], sel_1, immutable = skip_bound)
				second_reference=second_reference.getNext(insert_key.split("/")[-2], sel_1)

			else:

				replace_operator = second_reference.getNext(insert_key.split("/")[-1], selectivity).getRecommended()

			if feedback_list[n_rel][0] + feedback_list[n_rel][1] > 0:
				second_reference.updateKey(insert_key.split("/")[-1], feedback_list[n_rel], selectivity, immutable = skip_bound)
			second_reference=second_reference.getNext(insert_key.split("/")[-1], selectivity)
...
getPrefixCount(plan_cache, 0, "")
path_dict.pop(0)
#path_dict.pop(17)
print(path_dict)


with open("path_len_flat_plain.pkl","wb") as pf:
	pickle.dump( path_dict, pf)
