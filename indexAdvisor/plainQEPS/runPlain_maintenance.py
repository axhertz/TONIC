...
def getAttr(rel_target, rel_partner, e_file):
	
	#print(rel_target, rel_partner)
	if "#" in rel_partner:
		rel_partner = rel_partner.split("#")[-1]
	#print(rel_target, rel_partner)
	id_tar = ["it", "ct", "chn", "t","kt", "cct" ,"cn", "k", "n", "rt","chn", "lt"]
	ml_tar = ["lt", "t", "mi_idx", "ci"]
	if rel_target == "cct1": rel_target = "cct"
	if rel_target =="cct2": rel_target = "cct"
	#print(clean_data)
	if rel_target in id_tar: return "id"
	if rel_target == "ci" and rel_partner =="n": return "person_id"
	if rel_target == "ci" and rel_partner =="rt": return "role_id"
	if rel_target == "ci" and rel_partner =="chn": return "person_role_id"
	if rel_target == "ci": return "movie_id"
	if rel_target == "mc" and rel_partner == "cn": return "company_id"
	if rel_target == "mc" and rel_partner == "ct": return "company_type_id"
	if rel_target == "mc": return "movie_id"
	if rel_target == "mi" and rel_partner == "it": return "info_type_id"
	if rel_target == "mi_idx" and rel_partner == "it": return "info_type_id"
	if rel_target == "mi_idx": return "movie_id"
	if rel_target == "mk" and rel_partner == "k": return "keyword_id"
	if rel_target == "mk": return "movie_id"
	if rel_target == "mi_idx": return "movie_id"
	if rel_target == "mi": return "movie_id"
	if rel_target == "cn": return "movie_id"
	if rel_target == "an": return "person_id"
	if rel_target == "at": return "movie_id"
	if rel_target == "pi" and rel_partner =="an": return "person_id"
	if rel_target == "cc" and rel_partner == "cct":
		with open(e_file,"r") as ef:
			for line in ef:
				if ".subject_id" in line: return "subject_id"
				if ".status_id" in line: return "status_id"
	if rel_target == "cc":
		return "movie_id"
	if rel_target == "ml" and rel_partner == "t":
		with open(e_file,"r") as ef:
			if "ml.movie_id = t.id" in ef.read(): return "movie_id"
		return "linked_movie_id"
	if rel_target == "ml" and not rel_partner in ml_tar: return "movie_id"
	if rel_target == "pi" and rel_partner == "it": return "info_type_id"
	if rel_target == "pi": return "person_id"
	#if rel_target == "cc" and rel_partner  == "cct": return "movie_id"
...
for q in queries_sort:
	file_list.append("../../feedback/fullData/{}".format(q))


for file in file_list:

	e_file = "explicit"+file.split("/")[-1]
	...
	for n_rel, rels in enumerate(rel_matches):
		...
		if rels[0] != root_rel:
			...
			if len(rels) == 2:
				...
				reference_branch.updateKey("#"+insert_key.split("/")[-2], [0,0], sel_1, immutable = skip_bound)
				reference_branch=reference_branch.getNext("#"+insert_key.split("/")[-2], sel_1)
				reference_branch.attr = getAttr(insert_key.split("/")[-1],insert_key.split("/")[-2],e_file)
			else:
				replace_operator = reference_branch.getNext("#"+insert_key.split("/")[-1], selectivity).getRecommended()
			if feedback_list[n_rel][0] + feedback_list[n_rel][1] > 0: 
				reference_branch.updateKey("#"+insert_key.split("/")[-1], feedback_list[n_rel], selectivity, immutable = skip_bound)
...
with open("plain_qeps.pkl".format(plan_cache.gamma),"wb") as file:
	pickle.dump(plan_cache,file)
